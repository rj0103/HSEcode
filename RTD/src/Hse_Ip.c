...
#define HSE_IP_SW_MAJOR_VERSION_C             7
...
/* Macro for get Core Id in cases of User Mode and IP Version */
#if ((STD_ON == OSIF_ENABLE_USER_MODE_SUPPORT) && (defined (MCAL_ENABLE_USER_MODE_SUPPORT)))
    #define HSE_IP_TCM_CORE_INDEX       OsIf_Trusted_Call_Return(Hse_Ip_GetPhysicalCoreId)
#else
    #define HSE_IP_TCM_CORE_INDEX       Hse_Ip_GetPhysicalCoreId()
#endif /* ((STD_ON == OSIF_ENABLE_USER_MODE_SUPPORT) && (defined (MCAL_ENABLE_USER_MODE_SUPPORT))) */
...
/*!
 * @brief       Gets the physical core ID.
 * @details     This function gets the core Id of core running this application.
 *
 * @return      Core Id
 */
uint8 Hse_Ip_GetPhysicalCoreId(void)
{
    uint8 coreId = 0U;
...
)
{
    ...
    {
        if (u8MuChannel < (uint8) HSE_IP_NUM_OF_CHANNELS_PER_MU)
        {
...
)
{
    ...
    while (HSE_IP_INVALID_MU_CHANNEL_U8 != u8MuChannel)
    {
        ...
        /* Releasing the lock before releasing channel */
        if(TRUE == pHseIpMuState->boSemaphoreFlag[u8MuChannel])
        {
            pHseIpMuState->boSemaphoreFlag[u8MuChannel] = FALSE;
            (void) Hse_Ip_Semaphore_ReleaseLock(u8MuInstance, u8MuChannel);
        }
...
#if ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42))
        /* Mark the semaphore flag as not locked */
        pHseIpMuState->boSemaphoreFlag[u8MuChannel] = FALSE;
#endif /* ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42)) */
    }
...
)
{
...
#if (STD_ON == HSE_IP_ENABLE_TCM_SUPPORT)
    /* Get Core Index to determine which TCM address offset to apply */
    uint8 CoreIdx = Hse_Ip_GetPhysicalCoreId();
    ...
    {
        RetVal = (HOST_ADDR)(Address + Hse_Ip_aItcmAddrOffset[CoreIdx]);
    }
    else
    ...
    {
        RetVal = (HOST_ADDR)(Address + Hse_Ip_aDtcmAddrOffset[CoreIdx]);
    }
    else
...
)
{
    ...
    else
    {
...
#if ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42))
        /* Check if current request is cancel request */
        if (HSE_SRV_ID_CANCEL == (hseSrvId_t)pHseSrvDesc->srvId)
        {
            /* Check if lock is acquired by job and need to be released */
            if(TRUE == pHseIpMuState->boSemaphoreFlag[pHseSrvDesc->hseSrv.cancelSrvReq.muChannelIdx])
            {
                pHseIpMuState->boSemaphoreFlag[pHseSrvDesc->hseSrv.cancelSrvReq.muChannelIdx] = FALSE;
                (void) Hse_Ip_Semaphore_ReleaseLock(u8MuInstance, pHseSrvDesc->hseSrv.cancelSrvReq.muChannelIdx);
            }
            boSendSrvToHse = TRUE;
        }
        /* Check if the current service access flash on HSE FW side*/
        else if(TRUE == Hse_Ip_Semaphore_IsLockRequired((hseSrvId_t)pHseSrvDesc->srvId))
        {
            u32TimeoutTicks = Hse_Ip_MicrosToTicks_GetCounter_GetElapsed(HSE_IP_SEMAPHORE_TIMEOUT_U32, NULL_PTR, CRYPTO_HSE_IP_10_OSIF_MICROS_TO_TICKS_CALL);
            ...
            do
            {
                ...
                if(TRUE == Hse_Ip_Semaphore_RequestLock(u8MuInstance, u8MuChannel))
                {
                    /* Mark semaphore flag as true */
                    pHseIpMuState->boSemaphoreFlag[u8MuChannel] = TRUE;
                    boSendSrvToHse = TRUE;
                    /*Break in case lock acquired before timeout*/
                    break;
                }
                /* For HSE_IP_REQTYPE_ASYNC_IRQ or HSE_IP_REQTYPE_ASYNC_POLL requests, or if timeout expired - return Flashed locked */
                else if((u32ElapsedTicks >= u32TimeoutTicks) || (HSE_IP_REQTYPE_SYNC != pRequest->eReqType))
                {
                    Hse_Ip_ReleaseChannel(u8MuInstance, u8MuChannel);
                    HseResponse = HSE_IP_SRV_RSP_FLASH_LOCKED ;
                    boSendSrvToHse = FALSE;
                }
                else
                {
                    /*Do Nothing*/
            ...
            while((u32ElapsedTicks < u32TimeoutTicks) && (HSE_IP_REQTYPE_SYNC == pRequest->eReqType));
        }
        else
        {
            /* The current service does not access the flash on HSE FW side, therefore mark the flag as true */
            boSendSrvToHse = TRUE;
        }

        /* Check if the service can be sent to HSE FW, at this point does not matter if semaphore was required or not */
        if(TRUE == boSendSrvToHse)
#endif /* ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42)) */
        {
            ...
            if (HSE_IP_REQTYPE_SYNC == pRequest->eReqType)
            {
                ...
                u32CurrentTicks = Hse_Ip_MicrosToTicks_GetCounter_GetElapsed(0u, NULL_PTR, CRYPTO_HSE_IP_11_OSIF_GET_COUNTER_CALL);
                /* In case a service which requires semaphore was used elapsed time is different than 0 at this point
                    Therefore update the elapsed ticks to 0 */
                u32ElapsedTicks = 0;
                ...
                {
                    /* Check if HSE response is ready in case timeout happens because of interrupt */
...
#if (STD_ON == ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42)))
                /* Releasing the lock before releasing channel */
                if(TRUE == pHseIpMuState->boSemaphoreFlag[u8MuChannel])
                {
                    pHseIpMuState->boSemaphoreFlag[u8MuChannel] = FALSE;
        ...
        {
            /* HSE FW response already set to flash locked. Therefore nothing to do here */
        }
#endif /* ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42)) */
    }
