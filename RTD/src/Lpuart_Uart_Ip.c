...
#define LPUART_UART_IP_AR_RELEASE_MINOR_VERSION_C       9
#define LPUART_UART_IP_AR_RELEASE_REVISION_VERSION_C    0
#define LPUART_UART_IP_SW_MAJOR_VERSION_C               7
#define LPUART_UART_IP_SW_MINOR_VERSION_C               0
#define LPUART_UART_IP_SW_PATCH_VERSION_C               1
...
static boolean Lpuart_Uart_Ip_UpdateErrorIRQHandler(const uint8 Instance);
static boolean Lpuart_Uart_Ip_CheckErrorIRQHandler(const uint8 Instance, Lpuart_Uart_Ip_StatusFlagType Status_Flag, \
                                                   Lpuart_Uart_Ip_InterruptType Interrupt_Mode, Lpuart_Uart_Ip_StatusType Receive_Status);
                                                    ...
                                                    )
{
    ...
    else
    {
        ...
        LPUART_UART_IP_DEV_ASSERT(ClockFrequency >= ((uint32)DesiredBaudrate * 4U));
        ...
        Osr = 4;
        Sbr = (uint32)(ClockFrequency / ((uint32)DesiredBaudrate * Osr));

        CalculatedBaud = (ClockFrequency / ((uint32)Osr * (uint32)Sbr));
        if (CalculatedBaud > (uint32)DesiredBaudrate)
        {
            BaudDiff = CalculatedBaud - (uint32)DesiredBaudrate;
        }
        else
        {
            BaudDiff = (uint32)DesiredBaudrate - CalculatedBaud;
        }
        /* find maximum Osr */
        MaxOsr = ClockFrequency / (uint32)DesiredBaudrate;
        if (MaxOsr > 32U)
        {
            MaxOsr = 32U;
        }
        ...
        if (MaxOsr >= 5U)
        {
            for (Idx = 5U; Idx <= MaxOsr; Idx++)
            {
                ...
                SbrTemp = (uint32)(ClockFrequency / ((uint32)DesiredBaudrate * Idx));
                /* calculate the baud rate based on the temporary Osr and Sbr values */
                CalculatedBaud = (ClockFrequency / (Idx * (uint32)SbrTemp));

                if (CalculatedBaud > (uint32)DesiredBaudrate)
                {
                    TempDiff = CalculatedBaud - (uint32)DesiredBaudrate;
                }
                else
                {
                    TempDiff = (uint32)DesiredBaudrate - CalculatedBaud;
                }
        ...
        /* Check if the value of SBR is out of range the BAUD[SBR] */
        if (Sbr <= (uint32)LPUART_BAUD_SBR_MASK)
        {
        ...
        {
            Status = LPUART_UART_IP_STATUS_ERROR;
        }
    }
    return Status;
}
...
static void Lpuart_Uart_Ip_TxEmptyIrqHandler(const uint8 Instance)
{
    ...
    {
        /* Transmit empty flags (TDRE) can not be cleared without affecting to normal operation, return to exit */
    }
}
...
static void Lpuart_Uart_Ip_TxCompleteIrqHandler(const uint8 Instance)
{
    ...
    {
        /* Transmission complete flags (TC) can not be cleared without affecting to normal operation, return to exit */
    }
}
...
static void Lpuart_Uart_Ip_ErrIrqHandler(const uint8 Instance)
{
    ...
    IsReturn = Lpuart_Uart_Ip_UpdateErrorIRQHandler(Instance);
...
static void Lpuart_Uart_Ip_GetData(const uint8 Instance)
{
    ...
    if (UartState->RxSize > 0U)
    {
        ...
        else
        {
            ...
            {
                *(UartState->RxBuff) = (uint8)(Data16b & (uint16)UINT8_MAX);
                ++UartState->RxBuff;
                --UartState->RxSize;
            }
            else
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Lpuart_Uart_Ip_UpdateErrorIRQHandler
 * Description   : Update the receive status according to the error occurred for Lpuart Uart.
 *
 *END**************************************************************************/
static boolean Lpuart_Uart_Ip_UpdateErrorIRQHandler(const uint8 Instance)
{
    ...
    if(Lpuart_Uart_Ip_CheckErrorIRQHandler(Instance, LPUART_UART_IP_RX_OVERRUN, \
       LPUART_UART_IP_INT_RX_OVERRUN, LPUART_UART_IP_STATUS_RX_OVERRUN))
    {
        IsError = TRUE;
    }

    /* Handle framing error interrupt */
    if(Lpuart_Uart_Ip_CheckErrorIRQHandler(Instance, LPUART_UART_IP_FRAME_ERR, \
       LPUART_UART_IP_INT_FRAME_ERR_FLAG, LPUART_UART_IP_STATUS_FRAMING_ERROR))
    {
        IsError = TRUE;
    }

    /* Handle parity error interrupt */
    if(Lpuart_Uart_Ip_CheckErrorIRQHandler(Instance, LPUART_UART_IP_PARITY_ERR, \
       LPUART_UART_IP_INT_PARITY_ERR_FLAG, LPUART_UART_IP_STATUS_PARITY_ERROR))
    {
        IsError = TRUE;
    }

    /* Handle noise error interrupt */
    if(Lpuart_Uart_Ip_CheckErrorIRQHandler(Instance, LPUART_UART_IP_NOISE_DETECT, \
       LPUART_UART_IP_INT_NOISE_ERR_FLAG, LPUART_UART_IP_STATUS_NOISE_ERROR))
    {
        IsError = TRUE;
    }
    ...
    if(Lpuart_Uart_Ip_CheckErrorIRQHandler(Instance, LPUART_UART_IP_TIMEOUT, \
       LPUART_UART_IP_INT_TIMEOUT, LPUART_UART_IP_STATUS_RX_IDLE_STATE))
    {
        IsError = TRUE;
    }
#endif
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Lpuart_Uart_Ip_CheckErrorIRQHandler
 * Description   : Check and update the each receive error for Lpuart Uart.
 *
 *END**************************************************************************/
static boolean Lpuart_Uart_Ip_CheckErrorIRQHandler(const uint8 Instance, Lpuart_Uart_Ip_StatusFlagType Status_Flag, \
                                                   Lpuart_Uart_Ip_InterruptType Interrupt_Mode, Lpuart_Uart_Ip_StatusType Receive_Status)
{
    ...
    if (Lpuart_Uart_Ip_GetStatusFlag(Base, Status_Flag))
    {
        if (!Lpuart_Uart_Ip_GetIntMode(Base, Interrupt_Mode))
        {
            /* Do nothing - Return immediately */
        }
        else
        {
            ...
            if (UartState != NULL_PTR)
            {
                ...
                IsError = TRUE;
                UartState->ReceiveStatus = Receive_Status;
            }
            /* Clear the flag */
            Lpuart_Uart_Ip_ClearStatusFlag(Base, Status_Flag);
        }
    }
