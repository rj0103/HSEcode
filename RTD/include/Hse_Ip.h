...
#define HSE_IP_SRV_RSP_FLASH_LOCKED         ((hseSrvResponse_t)0xBB77BB77UL)
/* Semaphore timeout value */
#define HSE_IP_SEMAPHORE_TIMEOUT_U32        ((uint32)1000000000UL)
...
#if ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42))
    boolean                             boSemaphoreFlag[HSE_NUM_OF_CHANNELS_PER_MU];        /*!< Flag to indicate lock granted to Crypto  */
#endif /* ((STD_ON == HSE_IP_HARDWARE_SEMA42) || (STD_ON == HSE_IP_SOFTWARE_SEMA42)) */
} Hse_Ip_MuStateType;
...
/*!
 * @brief       Releases a channel allocated for HSE IP service request.
 * @details     This function releases the lock for an MU channel, making it available for other services.
 *              Hse_Ip_ServiceRequest() will also release the channel when called using it, so calling of Hse_Ip_ReleaseChannel() might not be required.
 *
 * @note        This function does not release HW channel acquired by HSE FW, it only release software lock for channel availablity.
 *              On completion of HSE service HW channel will be freed automatically.
 *              If releasing of HW channel is required HSE cancel request need to be sent.
 *
 * @param[in]   u8MuInstance    MU Instance number
