...
**Stage 2 — Register GSLU_APP's real flashed image as its own SMR entry**

This is "Region 1b" from the original `SECURE_BOOT_PLAN.md` (`__text_start`/`__text_end`, real code — not the throwaway Data Flash test region from Stage 1a), given its own SMR table index distinct from whatever index the bootloader's own image ends up using in Phase 2 below.
...
**Open decision — who installs this SMR, and when?** Don't silently pick one of these; they have different trust implications:
- **(a)** A one-time provisioning step, run by the bootloader itself (or a separate host tool) before GSLU_APP is ever trusted to touch its own SMR — the stronger model, since the thing being verified never gets to describe its own verification.
- **(b)** GSLU_APP installs/updates its own SMR on every boot, the same way Stage 1a's demo currently self-signs on-device — simpler, consistent with the "single dev board" simplification already called out in `SECURE_BOOT_PLAN.md`, but means the app can self-attest. Acceptable for continued bring-up, not for anything closer to production.
...
1. **Stage 2's "who installs the app's SMR" decision** (self-provisioned vs. externally provisioned) — pick one deliberately, don't default silently. This now also decides who holds the ECDSA private key, since whoever produces the (r,s) signature over GSLU_APP's image needs it.
2. Whether the bootloader's HSE bring-up needs its own MU channel bookkeeping independent of GSLU_APP's (they run at different times, never concurrently, so this is likely low-risk, but worth confirming against the HSE Reference Manual's guidance on per-boot-stage MU state).
